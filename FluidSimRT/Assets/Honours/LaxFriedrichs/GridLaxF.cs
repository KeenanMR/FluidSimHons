...
public class GridLaxF : MonoBehaviour
{
	//Grid size
	int xSize;
	...
	public void Generate()
	{
		...
		if (!GPUToggle.isOn)
		{
			LaxF script = gameObject.AddComponent<LaxF>();
			script.Init(xSize);
		}
		else
		{
			LaxFGPU script2 = gameObject.AddComponent<LaxFGPU>();
			script2.Init(xSize);
		}
	...
	public void RemoveScript()
	{
		if (!GPUToggle.isOn)
		{
			Destroy(GetComponent<LaxF>());
		}
		else
		{
			Destroy(GetComponent<LaxFGPU>());
		}
