...
public class Grid : MonoBehaviour
{
	...
	public void Generate()
	{
		...
		if (!GPUToggle.isOn)
		{
			BasicWaveEquation script = gameObject.AddComponent<BasicWaveEquation>();
			script.Init(xSize);
		}
		else
		{
			BasicWaveEquationGPU script2 = gameObject.AddComponent<BasicWaveEquationGPU>();
			script2.Init(xSize);
    ...
    public void RemoveScript()
    {
		if (!GPUToggle.isOn)
		{
			Destroy(GetComponent<BasicWaveEquation>());
		}
		else
		{
			Destroy(GetComponent<BasicWaveEquationGPU>());
		}
